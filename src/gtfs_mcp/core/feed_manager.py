    ...
    async def add_feed(
        ...
        if needs_update:
            try:
                ...
                logger.error(f"Failed to update feed {feed_id}: {str(e)}")
                if not feed_dir.exists() or not any(feed_dir.iterdir()):
                    raise GTFSValidationError(f"No valid feed data available for {feed_id}")
        ...
        try:
            ...
            logger.error(f"Failed to load feed {feed_id}: {str(e)}")
    ...
    async def _download_feed(
        ...
        with tempfile.TemporaryDirectory(prefix=f"gtfs_{feed_id}_") as temp_dir:
            ...
            try:
                ...
                raise GTFSValidationError(f"Invalid ZIP file: {str(e)}") from e
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise GTFSValidationError(f"Failed to download {url} after {max_retries} attempts: {str(e)}")
