...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10913 (fleet block; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10913
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10913")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10913)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10913

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10913 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10913)")
...
async def run_server_async(mcp_app, args: argparse.Namespace | None = None, server_name: str = "mcp-server") -> None:
    ...
    try:
        if transport == "stdio":
            ...
            # Fleet CORS standard: uvicorn.Server on mcp.http_app() so custom
            # middlewares survive (run_http_async drops CORSMiddleware).
            import uvicorn
