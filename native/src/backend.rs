...
const BACKEND_NAME: &str = "gtfs-mcp-backend.exe";
const BACKEND_PORT: u16 = 10913;
const BACKEND_TAG: &str = "gtfs-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "GTFS_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        let img_kill = format!(
            "Stop-Process -Name 'gtfs-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'gtfs-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM gtfs-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM gtfs-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                ...
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name gtfs-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM gtfs-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    log_line(
        &app,
        &format!("spawning {} (cwd {}) on port 10913",
    ...
    // Poll backend TCP port to confirm it is actually listening. Catches
    // processes that start but crash during module loading.
    ...
    Ok(format!("Backend starting on port 10913"))
}
